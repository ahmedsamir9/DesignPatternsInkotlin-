...
class TernarySearch:SearchingAlgorithm {
    override fun search(list: List<Int>, target: Int): Boolean {
        ...
        return recTernarySearch(newList,target,0,list.size-1)
    }
    fun recTernarySearch(list: List<Int>, target: Int, left:Int, right:Int):Boolean{
        ...
        if (list[mid1]> target )
            return recTernarySearch(list,target,left,mid1-1)
        if (list[mid2]< target)
            return recTernarySearch(list, target, mid2+1, right)
        return recTernarySearch(list, target, mid1+1,mid2-1)
    }
}
